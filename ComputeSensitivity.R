...
#' @export
computeSensitivity <- function(viability, concentration, fold.change=3, dilution.no=9, replicates, experiment.dim=c(11, 6), experiment.index.start=3, cutoff=2) {
  viability <- read.csv(viability, stringsAsFactors=FALSE, sep="\t", header=FALSE, na.strings=c("", " ", "NA"))
  ...
  for(index in indicies) {
    ...
    for(j in 1:data.blocks){
      ...
      for(dd in 1:length(drugs)) {
        ...
        raw.sensitivity[exp, , "Dose"] <- c(rep("pbs", times=data.blocks), doses, rep("control", times=data.blocks))
        pbs <- grep("pbs", raw.sensitivity[exp, , "Dose"])
        control <- grep("control", raw.sensitivity[exp, , "Dose"])
        treated <- match(doses, raw.sensitivity[exp, , "Dose"])
        
        raw.sensitivity[exp, pbs, paste0("Viability", 1:replicates)] <- matrix(mm[,1], ncol=replicates, nrow=data.blocks)
        raw.sensitivity[exp, control, paste0("Viability", 1:replicates)] <- matrix(as.numeric(mm[(nrow(mm) - (replicates - 1)): nrow(mm) , (ncol(mm) - (data.blocks - 1)): ncol(mm)]), ncol=replicates, nrow=data.blocks)
        tt <- t(mm[1:replicates, 2:(ncol(mm)-1)])
